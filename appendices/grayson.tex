...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{Screenshot 2026-03-23 at 2.40.48 PM.png}
...
\end{theorem}
This result is equivalent to the Rigidity Theorem, conjectured by Witten and proved by Bott and Taubes in \textbf{BOTT AND TAUBES}. A more direct and perhaps satisfying analogue is to note that in the case of $\xi: E \to B$ a complex vector bundle of dimension $k$, $\mathbb{CP}(\xi)$ its projectivization fiberwise [a fiber bundle over the unitary group $U(k)$], we can use multiplicativty of signature over fiber bndles to get $\text{signature} [\mathbb{CP}(\xi)] = \text{signature}(B) \text{signature}(\mathbb{CP}^{k-1})$. In the case where $k$ is even $\Rightarrow k - 1$ odd, this must equal $0$ since $\mathbb{CP}^{k-1}$ is a nullbordant space. Ochanine in \textbf{Ochanine}:

\begin{theorem}
    A genus $\p$ is ellptic if and only if $\p(\mathbb{CP}[\xi]) = 0$ for all $\xi$ even-dimensional complex vector bundles.
\end{theorem}
